...
const char * CLibScanner::COFFGetName(const IMAGE_SYMBOL* coff_sym, const char* coff_strtab)
{
...
bool CLibScanner::ScanCOFFObject(COFFOBJECT_LIST &objs,BYTE * lpBuffer,uint32_t Len)
{
...
bool CLibScanner::ScanLib(const char * szLib)
{
...
PFUNCTION_SYMBOL CLibScanner::GetFunctionInfo(const char * szFun)
{
        for(FUNCTION_LIST::iterator it = m_funs.begin();it!=m_funs.end();++it)
        {
                PFUNCTION_SYMBOL pFun= *it;
...
std::string CLibScanner::CheckIfLibFunc(PCBYTE phead)
{
    for(FUNCTION_LIST::iterator it = m_funs.begin();it!=m_funs.end();++it)
    {
        PFUNCTION_SYMBOL pFun= *it;
