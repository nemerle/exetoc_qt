...
Class_st* CClassManage::LoopUp_class_by_name(const char * name)
{
    CLASS_LIST::iterator pos = this->m_classlist.begin();
    for (;pos!=m_classlist.end();++pos)
    {
...
CFuncType* CClassManage::Get_SubFuncDefine_from_name(const char * classname, const char * funcname)
{
...
VarTypeID CClassManage::if_StrucName(const char * &pstr)
{
	CLASS_LIST::iterator pos = this->m_classlist.begin();
	for (;pos!=m_classlist.end();++pos)
	{
