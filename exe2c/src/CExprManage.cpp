...
const char * CExprManage::BareVarName(const VAR* v)
{
    if (v->type == v_Tem)
...
M_t* CExprManage::GetVarByName_1(MLIST* list, const char * varname)
...
M_t* CExprManage::GetVarByName(const char * varname)
{
...
void 	CExprManage::prt_var_Immed(const VAR* v, CXmlOutPro* out)
{
...
const char * CExprManage::VarName_Immed(const VAR* v)
{
    if (v->thevar == NULL || g_VarTypeManage->is_simple(v->thevar->m_DataTypeID))
...
const char * CExprManage::VarName(const VAR* v)
{
    assert(v->type != v_Tem);
...
void 	CExprManage::prt_var(const VAR* v, CXmlOutPro* out)
{
    if (v->type == v_Immed)
    {
        prt_var_Immed(v, out);
...
M_t* CExprManage::AddRef_immed(uint32_t d, uint32_t size)
{
...
M_t* CExprManage::AddRef_tem(uint32_t temno, uint32_t size)
{
    MLIST::iterator pos = vList->begin();
...
M_t* CExprManage::AddRef_with_name(en_MTTYPE type, uint32_t off, uint32_t size, const char * tj_name)
{
    assert(type != MTT_tem);
...
M_t* CExprManage::SearchMT(en_MTTYPE type, uint32_t off)
{
    MLIST::iterator pos = vList->begin();
    for (;pos!=vList->end();++pos)
    {
