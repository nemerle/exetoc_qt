...
void FileLoader::readnefile(uint32_t offs)
{
}
...
/*
void FileLoader::readpefile(uint32_t peoffs)
{
              ...
              (peh->exporttable_rva < pdata[i].rva+pdata[i].phys_size)) )
                {
                        ...
                        uint32_t numsymbols=expdir->numfunctions;
                        ...
                        for ( k=0;k<peh->objects;k++ )
                        {
                                ...
                                {
                                        nnaddr=(uint32_t *)&fbuff[expdir->nameaddrrva-pdata[k].rva+pdata[k].phys_offset];
                                        break;
                                }
                        }
                        for ( k=0;k<peh->objects;k++ )
                        {
                                ...
                                {
                                        fnaddr=(uint32_t *)&fbuff[expdir->funcaddrrva-pdata[k].rva+pdata[k].phys_offset];
                                        break;
                                }
                        }
                        for ( k=0;k<peh->objects;k++ )
                ...
                {
                        uint32_t *imphint,impaddr,impaddr2;
                        BYTE* impname;
                        ...
                        while ( impdir[j].firstthunkrva )
                        {
                                ...
                                for ( k=0;k<peh->objects;k++ )
                                {
                                        ...
                                        {
                                                imphint=(uint32_t *)&fbuff[thunkrva-pdata[k].rva+pdata[k].phys_offset];
                                                break;
                                        }
                                }
                                ...
                                while ( *imphint )
                                {
                                        ...
                                        {
                                                uint32_t numtmp=(*imphint)&0x7fffffff;
                                                wsprintf(inum,"%02d",numtmp);
                ...
                { // RESOURCE_DATA;
                        if ( (pdata[i].phys_size)&& Load_Resources )
                        {
                                ...
                                rawdata=(BYTE *)resdir;
                                uint32_t numitems=resdir->numnames+resdir->numids;
                                ...
                                while ( numitems )
                                {
                                        ...
                                        else
                                        {
                                                ...
                                                default:
                                                        ...
                                                        uint32_t numtmp=rentry->id&0x7fffffff;
                ...
                if ( pdata[i].rva==peh->fixuptable_rva ) // fixup info
                {
                        ...
                        while ( per->rva )
                        {
                                ...
                                uint32_t numrelocs=(per->len-sizeof(perelocheader))/2;
                                while ( (numrelocs)&&(rdata[0]) )
                                {
                                        t.assign(SEG0,((uint32_t)(rdata[0])&0x0fff)+per->rva+peh->image_base);
                ...
                { // INIT_DATA;
                        ...
                        if ( (pdata[i].virt_size>pdata[i].phys_size)&& Load_Data )
                        {
                                ...
                                for ( uint32_t j=0;j<pdata[i].virt_size-pdata[i].phys_size;j++ )
                                        uinit[j]=0;
                ...
                { // UNINIT_DATA;
                        if ( Load_Data )
                        {
                                ...
                                for ( uint32_t j=0;j<pdata[i].virt_size;j++ )
                                        uinit[j]=0;
