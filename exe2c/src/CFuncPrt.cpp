...
void	CFunc_Prt::prt_case(const INSTR * phead, const INSTR * plabel, CXmlOutPro* out)
{
...
void	CFunc_Prt::prt_add(const INSTR *p, const char * s, CXmlOutPro* out)
{
    if (strcmp(s,"*") == 0)
    {
...
void    CFunc_Prt::out_PointTo(st_InstrAddOn* pa,const VAR* pv, CXmlOutPro* out)
{
    if (pv->thevar != NULL)
    {
...
void 	CFunc_Prt::prt_va_1(const st_InstrAddOn* pa, const VAR* pv, CXmlOutPro* out)
{
...
void 	CFunc_Prt::prt_va(const VAR_ADDON& va, CXmlOutPro* out)
{
...
void 	CFunc_Prt::prt_var(const VAR* var, CXmlOutPro* out)
{
    if (!IfTemVar(var))
    {
...
void	CFunc_Prt::prt_one_statement(const INSTR *phead, CXmlOutPro* out)
{
    if (phead == NULL)
        return;
...
void	CFunc_Prt::prt_switch_case(CasePrt_List* list, const INSTR* phead, CXmlOutPro* out)
{
...
void	CFunc_Prt::prt_compare(const INSTR * phead, CXmlOutPro* out)
// 打印while()括号中的东西。
//	特点：不换行，最后不分号
{
        // 暂时先这样
...
void	CFunc_Prt::prt_one_statement_mainbody(const INSTR * phead, CXmlOutPro* out)
{	//	不包括{和},用于case
    if (phead == NULL)
        return;
    POSITION pos = std::find(m_instr_list.begin(),m_instr_list.end(),phead);
...
void	CFunc_Prt::prt_the_instr_1(const INSTR *p, CXmlOutPro* out)
{
    switch (p->type)
    {
        case i_Cmp:
...
void	CFunc_Prt::prt_sub(const INSTR *p, const char * s, CXmlOutPro* out)
{
    if (IfTemVar(&p->var_w))
    {
        out->prtt("(");
...
void	CFunc_Prt::prt_iAddress_out(const INSTR * p, CXmlOutPro* out)
{
    int n=0;
    if (p->var_r1.type)
        {
...
void CFunc_Prt::prt_parameter(const INSTR * ppara, CXmlOutPro* out)
{
...
#undef m_funcname
void CFunc_Prt::prt_instr_call(const INSTR * p, CXmlOutPro* out)
{
    if (p->type == i_CallApi)
    {
...
void	CFunc_Prt::prt_instr(const INSTR * p, POSITION &nextpos, CXmlOutPro* out)
{
    CFunc_InstrList instrl(m_instr_list);
